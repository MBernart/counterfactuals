...
class CFRLCelebA(Explainer):
    def __init__(self, latent_dim=128, coeff_sparsity=0.5, coeff_consistency=0.5, train_steps=200, batch_size=32):
    ...
    def fit(self, model: Model, data: Dataset) -> None:
        ...
        if ae_path is None:
            raise FileNotFoundError("Could not find 'autoencoder_celeba_best.pth' in expected locations.")
        ...
        if is_latent_input:
            print("CFRLCelebA: Detected 2D input (latent). Reconstructing images for training...")
            # Reconstruct images from latent vectors
            # Warning: This expands dataset to images in memory.
            with torch.no_grad():
                latent_tensor = torch.tensor(data.data, dtype=torch.float32).to(self.device)
                ...
                print(f"CFRLCelebA: Reconstructed {len(X_train)} images with shape {X_train.shape[1:]}.")

            # Predictor Wrapper for Latent Input Model:
            # Image -> Encoder -> Latent -> User Model
            def predictor_wrapper(x: np.ndarray) -> np.ndarray:
                # x is (N, 3, 128, 128)
                with torch.no_grad():
    ...
    def explain(self, model: Model, data: Dataset) -> list[Counterfactual]:
        ...
        explanation = self.explainer.explain(X=X_explain, Y_t=targets, batch_size=self.batch_size)
        ...
        if explanation.cf is not None and 'X' in explanation.cf:
            ...
            if len(cf_X) > 1:
                ...
                print(f"CFRLCelebA Debug: Difference between first two CF images: {diffs}")
                unique_cfs = np.unique(cf_X.reshape(len(cf_X), -1), axis=0)
                print(f"CFRLCelebA Debug: Number of unique CFs generated: {len(unique_cfs)} out of {len(cf_X)}")
