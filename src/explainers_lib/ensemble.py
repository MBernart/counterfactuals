...
class Ensemble:
    def __init__(
        ...
        self.explainers = list(
            filter(
                lambda explainer: not isinstance(explainer, CeleryExplainer), explainers
            )
        )
        self.celery_explainers = list(
            filter(lambda explainer: isinstance(explainer, CeleryExplainer), explainers)
        )
    ...
    def explain(self,
        ...
        if task:
            ...
            if isinstance(results, list):
                cfs = [
                    Counterfactual.deserialize(counterfactual)
                ...
                cfs = [
                    Counterfactual.deserialize(counterfactual)
                    for counterfactual in results["counterfactuals"]
                ]
            else:
        ...
        if pretty_print:
            print_cfs(
                ...
                data=data,
                model=self.model,
                explainers=self.get_explainers_repr(),
                postprocess=pretty_print_postprocess,
                postprocess_target=pretty_print_postprocess_target,
...
def cfs_group_by_original_data(
    cfs: List[Counterfactual],
) -> Dict[bytes, List[Counterfactual]]:
...
def postprocess_cfs(
    ...
    postprocess_target: Postprocessor = lambda x: x,
) -> List[Counterfactual]:
...
def print_cfs(
    ...
    feature_names = (
        ...
        if feature_names
            ...
            data.features if data else np.arange(cfs[0].original_data.shape[0]).tolist()
        )
    )
    ...
    for bytes, cfs in by_instance.items():
        ...
        original_class = (
            cfs[0].original_class
            if len(cfs) > 0
            else model.predict(np.array([original_data]))[0] if model else None
        )
        original_class = (
            postprocess_target(np.array([original_class]))[0]
            if postprocess_target
            else original_class
        )
        original_class = (
            repr(int(original_class)) if original_class is not None else "N/A"
        )
        original_data = (
            postprocess(np.array([original_data])).iloc[0]
            if postprocess
            else original_data
        )
        ...
        table.add_row(
            *map(
                lambda x: "{:.4f}".format(x) if isinstance(x, (int, float)) else str(x),
                original_data,
            ),
            original_class,
            "original data",
        ...
        for explainer, cfs in sorted(by_explainer.items(), key=lambda items: items[0]):
            if len(cfs) > 0:
                for cf in cfs:
                    cf_data = (
                        postprocess(np.array([cf.data])).iloc[0]
                        if postprocess
                        else cf.data
                    )
                    cf_data = cf_data.tolist()
                    cf_target = (
                        postprocess_target(np.array([cf.target_class]))[0]
                        if postprocess_target
                        else cf.target_class
                    )
...
def ensure_celery_explainers(
    requested_explainers: List[CeleryExplainer],
) -> List[CeleryExplainer]:
    ...
    explainers = list(
        filter(
            lambda explainer: explainer.explainer_name in available_explainers,
            requested_explainers,
        )
    )
...
def fit_celery_explainers(
    explainers: List[CeleryExplainer], model: Model, data: Dataset
) -> Optional[Any]:
    if not explainers:
        return None
    ...
    return chain(
        group(fit_chains), app.signature("ensemble.collect_results")
    ).apply_async()


def explain_celery_explainers(
    explainers: List[CeleryExplainer], model: Model, data: Dataset
) -> Optional[Any]:
    if not explainers:
        return None
    ...
    return chain(
        group(explain_chains), app.signature("ensemble.collect_results")
    ).apply_async()
...
def repr_celery_explainers(explainers: List[CeleryExplainer]) -> Optional[Any]:
    ...
    return chain(
        group(repr_group), app.signature("ensemble.collect_results")
    ).apply_async()
